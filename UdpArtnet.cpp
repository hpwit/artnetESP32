...
uint8_t WiFiUDPArtnet::begin(IPAddress address, uint16_t port){
    stop();
...
uint8_t WiFiUDPArtnet::begin(uint16_t p){
    return begin(IPAddress(INADDR_ANY), p);
}

uint8_t WiFiUDPArtnet::beginMulticast(IPAddress a, uint16_t p){
...
int WiFiUDPArtnet::beginMulticastPacket(){
...
int WiFiUDPArtnet::beginPacket(IPAddress ip, uint16_t port){
    remote_ip = ip;
    remote_port = port;
    return beginPacket();
}

int WiFiUDPArtnet::beginPacket(const char *host, uint16_t port){
...
size_t WiFiUDPArtnet::write(uint8_t data){
...
size_t WiFiUDPArtnet::write(const uint8_t *buffer, size_t size){
    size_t i;
    for(i=0;i<size;i++)
        write(buffer[i]);
    return i;
...
int WiFiUDPArtnet::available(){
    if(!rx_buffer) return 0;
    return rx_buffer->available();
}

int WiFiUDPArtnet::read(){
...
int WiFiUDPArtnet::read(unsigned char* buffer, size_t len){
    return read((char *)buffer, len);
}

int WiFiUDPArtnet::read(char* buffer, size_t len){
    if(!rx_buffer) return 0;
...
int WiFiUDPArtnet::peek(){
    if(!rx_buffer) return -1;
    return rx_buffer->peek();
}

void WiFiUDPArtnet::flush(){
    if(!rx_buffer) return;
    cbuf *b = rx_buffer;
    rx_buffer = 0;
    delete b;
...
IPAddress WiFiUDPArtnet::remoteIP(){
    return remote_ip;
}

uint16_t WiFiUDPArtnet::remotePort(){
    return remote_port;
}
